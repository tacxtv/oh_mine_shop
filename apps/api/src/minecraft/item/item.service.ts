...
export class ItemService /* implements OnModuleInit */ {
  ...
  public async listAll() {
    const files = await readdirSync(`../../storage/render/`)
    const items = files.map((file) => {
      const [mod, name] = file.replace(/.png$/i, '').split('__')
      return {
        mod,
        name,
      }
  ...
  public async getOne(modId: string) {
    const files = await readdirSync(`../../storage/render/`)
    const items = files
      .filter((file) => file.startsWith(`${modId}__`))
      .map((file) => {
        const [mod, name] = file.replace(/.png$/i, '').split('__')
        return {
          mod,
  ...
  public async getMods() {
    ...
    const mods = [...new Set(items.map((item) => item.mod))]
    return mods
  }
